...
void Application::prepareFBO() {
	...
	glTexStorage2D(  GL_TEXTURE_2D, 1, GL_DEPTH24_STENCIL8, 2048, 2048);
	glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
...
void Application::draw() {
  ...
  GLint transform_location = glGetUniformLocation(quad_program, "transform");
  glm::mat4 transform = glm::mat4(0.25f); // glm::translate(glm::vec3(-6.25f, -0.25f, 0.0f)); //;* glm::mat4(0.25f);
  transform[0].x = 0;	transform[0].y = 0.25f;
  transform[1].x = -0.25f;	transform[1].y = 0;
  transform[2].z = 1.0f;
  transform[3].a = 1.0f;
  transform[3].x = -0.5f;
  glUniformMatrix4fv(transform_location, 1, GL_FALSE, (const GLfloat*)&transform[0][0]);
  ...
  transform[0].x = 0.25f;	transform[0].y = 0;
  transform[1].x = 0;	transform[1].y = 0.25f;
  transform[3].x = +0.0f;
  transform[3].y = +0.5f;
  glUniformMatrix4fv(transform_location, 1, GL_FALSE, (const GLfloat*)&transform[0][0]);
  glDrawArrays(GL_TRIANGLE_STRIP, 0, 4);
...
void Application::compileShaders() { 
  ...
  simple_program         = compile_link_vs_fs("../../src/glsl/simple.vert", "../../src/glsl/simple.frag");
  quad_program           = compile_link_vs_fs("../../src/glsl/quad.vert", "../../src/glsl/quad.frag");
}
