...
int Application::m_rendermode = 0;
...
void Application::init(const unsigned int& width, const unsigned int& height) {
    ...
    m_window = glfwCreateWindow(width, height, "Primitive Rendering Performance Comparsion", NULL, NULL);
    if (!m_window)
    {
        glfwTerminate();
        exit(EXIT_FAILURE);
...
void Application::create() {
   ...
   std::uniform_real_distribution<float> vertices_distribution(-2.0, 2.0);
   std::uniform_real_distribution<float> color_distribution(0.0, 1.0);

   std::vector<float> vertices, colors;
   for (size_t i = 0; i < num_points; i++) {
	   vertices.push_back(vertices_distribution(generator));
	   vertices.push_back(vertices_distribution(generator));
	   vertices.push_back(vertices_distribution(generator));
...
void Application::draw() {
  ...
  case 2:
	  glDrawArrays(GL_TRIANGLE_STRIP, 0, static_cast<GLsizei>(num_points));
	  break;
  };
...
void Application::EventKey(GLFWwindow* window, int key, int scancode, int action, int mods) {
  ...
  if (action == GLFW_RELEASE) {
	  ...
	  if (key == GLFW_KEY_SPACE)                   m_rendermode = (m_rendermode+1) % 3;
